            ...
            <div class="container-fluid">
                <div class="card">
                    <div class="header">
                        ...
                        <h5><center>Doll specifications</h5>
...
$queryproduct="SELECT OProductID FROM Orders WHERE OrderID=$orderID";
...
if ($row=mysqli_fetch_array($resultproduct)){
    $productID = $row['OProductID'];
                                ...
                                $query4 = "Select p.productID, av.ValueName as 'valname', av.ValueImage as 'valimage', a.AttributeName as 'attname', pd.ProductName as 'prodname', pd.ProductType as 'prodtype' from product_has_attribute p join attributevalues av on p.AttributeValueID = av.ValueID join attribute a on av.AttributeTypeID = a.AttributeID join product pd on pd.ProductID = p.ProductID where p.productID =$productID order by a.AttributeName asc";
...
?>
<td width=50%><div align=center><img src=<?php  echo '"data:image/jpeg;base64,'.base64_encode( $row5['valimage'] ).'" '; ?></div></td>
...
echo 
"
<form action=\"dollOrders.php\" method=\"post\">
...
&nbsp;
<form action="reviewOrders.php?id=<?php echo $orderID; ?>" method='post'>
<?php 
echo 
"
<button type = \"submit\" name =\"reject\" class=\"btn btn-danger btn-fill pull-left\" value=\"".$id."\">REJECT</button>
<input type = \"hidden\" name =\"id\" class=\"\" value=\"".$id."\">
...
<a href="reviewOrders.php"><button type="submit" class="btn btn-warning btn-fill pull-right">Back to Review</button></a>
